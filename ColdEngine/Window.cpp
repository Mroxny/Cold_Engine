	...
	height(height)
{
	...
	{
		throw ColdWindowException(__LINE__, __FILE__);
	}
...
{
	if (SetWindowText(hWnd, convertCharsToLPWSTR( title.c_str())) == 0)
	{
		throw ColdWindowException(__LINE__, __FILE__);
	}
}
...
{
	return "Cold Window Exception <No Graphics>";
}

LPWSTR Window::convertCharsToLPWSTR(const char* text) {
